...
public class PowerStepListener implements StepExecutionListener {
    ...
    public ExitStatus afterStep(StepExecution stepExecution) {
        ...
        if (batchStartTime != null) {
            // 데이터 조회
            List<PowerDocument> documents = powerDocumentRepository
                    .findByCreatedAtBetweenAndIsProcessedFalse(batchStartTime, batchEndTime);

            if (!documents.isEmpty()) {
                // 모든 데이터를 isProcessed = true로 변환
                List<PowerDocument> updatedDocuments = documents.stream()
                ...
                // 일괄 저장 (한 번에 저장)
                powerDocumentRepository.saveAll(updatedDocuments);

                log.info("Power {} 개 처리됨", documents.size());
            } else {
                log.info("처리할 Power 데이터 없음");
            }
        }
